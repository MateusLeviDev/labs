...
public class MessageRoutingConfig {
    ...
    private final Map<String, String> routingFunctions = Map.of(
            "CustomerCreated", "processCustomerCreated",
            "EmailUpdated", "processEmailUpdated");
    ...
    public Consumer<Message<?>> unknownEvent() {
        return message -> log.warn("consuming unknown event, so the application will discard it: {}",
                message.getHeaders());
    }
}
